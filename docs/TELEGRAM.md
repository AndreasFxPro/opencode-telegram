...
`/mission` (alias `/control`) opens the read-only Mission Control overview. `/projects`, `/queue`, and `/inbox` open its project registry, active work queue, and operator inbox directly. These views follow the same Rich Message fallback, expiry, authorization, topic, user, and exact-message binding rules as the session dashboard. Mission Control callbacks only navigate or refresh the view; approval actions remain in their exact approval messages and work-state mutations remain in the hub CLI.
